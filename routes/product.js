...
//  -----------------------------------  상품리스트 기능 -----------------------------------------
// (회원 view) 등록된 상품리스트를 브라우져로 출력합니다.
const PrintCategoryProd = (req, res) => {
  ...
  const  query = url.parse(req.url, true).query;
       ...
       if (req.session.auth)   {   // 로그인된 경우에만 처리한다
           ...
           sql_str = "SELECT maker, pname, modelnum, rdate, price, pic from u18_products where category='" + search_cat + "' order by rdate desc;"; // 상품조회SQL
